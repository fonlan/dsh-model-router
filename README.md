...
- **快速路由切换**：当当前会话选择的是 `model-router` 模型时，输入栏模型选择器右侧显示当前 provider 按钮；点击打开 provider 面板，可直接切换该模型的激活 provider。切换对所有会话的后续请求生效，完整排序和全量管理仍在设置 → 模型路由中完成。可在设置 → 模型路由中通过「在输入栏显示快速路由切换按钮」开关控制该按钮是否显示。
